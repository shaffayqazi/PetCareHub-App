...
class ProductDetailController extends GetxController {
  ...
  Rx<CartListData> cartArgumentsList = CartListData(qty: 1.obs, productVariation: VariationData(inCart: false.obs), productReviewData: ProductReviewDataModel()).obs;
  ...
  void init() {
    if (Get.arguments is ProductItemData) {
      ...
      cartArgumentsList(Get.arguments as CartListData);
    }
    getProductDetails(isFromSwipRefresh: true);
    getReviewList();
  }
  ...
  getProductDetails({bool isFromSwipRefresh = false}) {
    ...
    productDetailsFuture(ShopApi.getProductDetails(productId: cartArgumentsList.value.productId.isNegative ? argumentsList.value.id : cartArgumentsList.value.productId)).then((value) {
  ...
  getReviewList() {
    getreview(ShopApi.productAllReviews(
      productId: cartArgumentsList.value.productId.isNegative ? argumentsList.value.id : cartArgumentsList.value.productId,
      page: page,
      list: allReviewList,
      lastPageCallBack: (p0) {
        isLastPage(p0);
